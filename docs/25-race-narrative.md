...
### 리타이어 — 랩 정체로 감지한다 (실측 확정)

**실측 (벨기에 GP 픽스처, 구현 전 관찰)**: 두 후보를 측정했다.

- **`sessionResults`(dnf/dns/dsq)**: 세션 종료 후에만 채워진다(docs/22 §B). 라이브 스냅샷의
  `retired` 는 하드코딩 `false`. → 라이브엔 없음, 아카이브 전용
- **`raceControl` 메시지 (원안 (a))**: **폐기.** 벨기에 리타이어 3명(STR·PER·RUS) 중
  race_control 에서 감지되는 건 **0명**이다. "STOPPED"·"RETIRED"·블랙플래그 문구 자체가
  피드에 없다. OpenF1 race_control 은 리타이어를 알리지 않는다

**채택: 랩 정체(lap staleness) 감지.** 측정에서 리타이어 차의 최종 완주 랩(STR=26·PER=14·
RUS=1)이 리더 랩(44)보다 확연히 뒤처지는 것으로 깔끔하게 분리됐다. 규칙:

- nowMs 시점의 현재 레이스 랩(리더 랩)이 어떤 드라이버의 **마지막 완주 랩보다 K랩 이상**
  앞서 있고, 그 드라이버가 **1랩 이상 달렸으며(합류 즉시 오검 방지)**, **피트 중이 아니면**
  → 리타이어로 본다. reason 은 없음(라이브엔 원본 없음), lap 은 마지막 완주 랩
- **K=2~3 (구현 시 픽스처로 확정)**. 이미 쓰는 패턴 — 추월 트래커의 "지속 침묵 후 확정"과
  같은 원리다
- **오검 안전성(측정으로 확인)**: 긴 피트(25초)도 인랩·아웃랩을 완주하므로 랩이 계속 는다 →
  정체 안 됨. SC·레드플래그는 리더도 함께 느려져 상대 랩 진행이 유지됨 → 전원 오검 없음
- 세션 종료 후 아카이브는 `sessionResults` 가 더 정확하므로 그쪽을 우선(라이브=정체 감지,
  아카이브=sessionResults). 회귀 테스트는 **라이브 정체 경로로** 검증해 종료 데이터 착시를
  피한다(수용 기준 §10)
