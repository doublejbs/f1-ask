...
## 개정 — 추월 예측·타이어 전략 적재 (C1·C4)

질문 컨텍스트에 워커 집계(피트·스틴트·추월)에 더해 두 가지를 싣는다. 세 provider 공용 함수
`toQuestionSummaryContext(snapshot)` 한 곳에서 하므로 컨텍스트가 갈라지지 않는다.

- **추월 예측(C4)**: 스냅샷 `overtakeForecasts` → `{ chaser, target, inLaps, confidence, gapSeconds }`.
  "누가 곧 추월?" 에 답한다.
- **타이어 전략(C1)**: 드라이버별 `usedCompounds`(신품/중고) + 규정 기반 잔여 추정
  (`setsRemaining` + 컴파운드별 범위). 잔여는 이번 레이스 신품 사용분 + 레이스 의무 보유로
  좁힌다(docs/29). "X 타이어 몇 세트 남았어?" 에 답한다.
- 전부 결정론 — LLM 이 수치를 지어내지 않는다. 데이터가 없으면(mock·replay) null.
