...
| `OvertakeModeEnabled` | `overtake_mode_enabled` | `OVERTAKE ENABLED` | Medium | — |
| `OvertakeModeDisabled` | `overtake_mode_disabled` | `OVERTAKE DISABLED` | Medium | — |
...
> OpenF1은 `OVERTAKE ENABLED/DISABLED` 문구를 계속 보낸다. 무선 용어가 아니다.
> DRS 는 2026 시즌부터 폐지되었으므로 이 문구는 DRS 가 아니라 **매뉴얼 오버라이드**
> (앞차와 1초 이내인 추격 차량에게 주어지는 전기 부스트) 사용 가능 구간을 뜻한다.
> UI 번역에서는 "오버테이크 모드"로 표기한다.
...
- `car_data`(RPM/기어 등), `location`(미니맵) — 3.7Hz 폴링이 필요하다.
